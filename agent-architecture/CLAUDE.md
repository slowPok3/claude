...
- **`Principal-Solution-Architect.md`** is the top-level orchestrator persona: it decomposes a request, delegates to the appropriate domain architect(s), and synthesizes their output, without doing domain-specific work itself. Its "Non-Redundancy Protocol" is a key rule: it must summarize how a sub-agent's output fits the broader architecture rather than repeating code/explanations the sub-agent already gave directly to the user, unless that sub-agent ran silently in the background. **This file is intentionally not installed as a Claude Code subagent** (it's absent from `.claude/agents/`) — a Claude Code subagent cannot invoke other subagents, so the delegation behavior this persona describes has no way to actually execute if loaded as one. That role belongs to the main Claude Code session itself; its guidance was rewritten into root `CLAUDE.md` instead. This file remains here for its original use case: pasting as a system prompt into a tool without native subagent orchestration.
- **`domain-architects/*.md`** are independent, self-contained system prompts, one per technology (Ansible, GitLab, ICAM/Zero Trust, Java, PingIdentity, Power BI, PowerShell, Python, RadianLogic, SailPoint). Each is meant to be used standalone (pasted directly as a system prompt, or as a Claude Code subagent) OR referenced/delegated to conceptually by the Principal Solution Architect persona when that persona is pasted elsewhere.
