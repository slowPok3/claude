...
2. **Title + emoji header**, immediately followed by a single `**Version:** X.Y.Z · **Compatibility:** ... · **Review Cycle:** ...` line — each agent has a distinct emoji used consistently in headers throughout the file. This is the **only** place the version number appears in the body (plus the frontmatter field — two locations total, not more). It stays visible at the top rather than buried in a bottom footer, since the frontmatter is stripped when someone copies the body into another AI tool (see root `README.md`), and because a version number nobody can find isn't one that gets updated. Do **not** add a second version mention elsewhere in the file (no bottom "Version & Maintenance" footer, no "Major Changes in vX.X.X" section) — that content belongs in `CHANGELOG.md`, which already tracks per-agent version history.
